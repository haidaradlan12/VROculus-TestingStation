...
public class MetaXRSimulatorEnabler : MonoBehaviour
{
    ...
    const string PreviousXrSimConfigEnvKey = "__PREVIOUS_META_XRSIM_CONFIG_JSON";

    const string PackagePath = "Packages\\com.meta.xr.simulator";

    private static string GetSimulatorJsonPath()
    {
        return Path.GetFullPath(PackagePath + "\\meta_openxr_simulator.json");
    }

    private static string GetSimulatorDllPath()
    {
        return Path.GetFullPath(PackagePath + "\\SIMULATOR.dll");
    }
    ...
    {
        return Path.GetFullPath(PackagePath + "\\config\\sim_core_configuration.json");
    }
    ...
    const string kActivateSimulator = "Oculus/Meta XR Simulator (Experimental, UPM)/Activate";
    ...
    const string kDeactivateSimulator = "Oculus/Meta XR Simulator (Experimental, UPM)/Deactivate";
